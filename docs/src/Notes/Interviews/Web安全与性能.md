...
**补充**，当数据传输完毕之后，浏览器与服务器会通过**四次挥手**来关闭连接。
...
在一个 CSRF 攻击场景中，加入你现在登录了 bank.com 这个网站，网站中记录了你的 cookie 信息，然后你又打开了一个标签页，在这个标签页中，你访问了一个恶意网站 evil.com 👾，这个网站中存在一些恶意脚本，脚本会偷偷地向 bank.com 发送伪造的请求，请求会自动携带上 bank.com 的 cookie 信息。这样依赖，恶意网站就可以利用这些信息施展它的恶意行动，比如神不知鬼不觉地转移你的资金 💸。
...
- **无连接**：默认情况下，每个 HTTP 请求都会创建一个新的 TCP 连接，并且在请求完成后关闭此连接，这导致了较高的延迟和较低的效率，尤其是在网页包含多个资源时。
...
- **Connection ID 实现切网不重握**：通常情况下，发送网络切换（例如从 WIFI 切到 4G）时，浏览器需要进行重新握手连接。QUIC 引入 Connection ID，通过 Connection ID 将不同网络的请求与响应关联起来 🦄，从而实现切网不重握。
