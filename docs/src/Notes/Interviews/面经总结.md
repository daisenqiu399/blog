---
updateTime: "2023-09-16 19:21:30"
---
# 前端面试考点
...
- `opacity:0` 占据空间，可以点击，可以设置过渡动画，（既不回流也不重绘，触发的是CSS3硬件加速）

> CSS3硬件加速通过将特定的CSS属性应用于元素，例如`transform`、`opacity`、`filter`等，浏览器会将这些元素的渲染和动画操作交给GPU来处理，从而提高性能和流畅度。
...
- 微任务: `Promise`的回调函数(Promise本身是同步的)，`async / await`
...
2. 在函数之中, this 的指向取决于函数的调用方式
   ...
   2. 如果函数使用 call / bind/ apply 进行调用,this指向作为参数传递的对象.
